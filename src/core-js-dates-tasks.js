...
function getNextFriday(date) {
  const diffs = [5, 4, 3, 2, 1, 7, 6];

  return new Date(date.setDate(date.getDate() + diffs[date.getDay()]));
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  return Math.abs(Date.parse(dateStart) - Date.parse(dateEnd)) / 86400000 + 1;
}
...
function getWeekNumberByDate(date) {
  const daysInterval =
    Math.ceil(
      (Date.parse(date) - Date.parse(date.getFullYear(), 0, 1)) / 86400000
    ) + 1;
  const firstWeekOffset =
    (new Date(date.getFullYear(), 0, 1).getDay() || 7) - 1;

  return Math.ceil((daysInterval + firstWeekOffset) / 7);
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  const startDateStr = period.start.split('-').reverse().join('-');
  const endDateStr = period.end.split('-').reverse().join('-');
  const start = new Date(startDateStr);
  const end = new Date(endDateStr);
  const result = [];

  result.push(formatDateIntl(start));
  while (start < end) {
    for (let i = 1; i < countWorkDays; i += 1) {
      result.push(formatDateIntl(start.setDate(start.getDate() + 1)));
    }
    const next = new Date(start.setDate(start.getDate() + countOffDays + 1));
    if (next <= end) {
      result.push(formatDateIntl(next));
    }
  }
...
function isLeapYear(date) {
  return (
    !(date.getFullYear() % 4) &&
    (!!(date.getFullYear() % 100) || !(date.getFullYear() % 400))
  );
}
